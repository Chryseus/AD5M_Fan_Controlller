...
void connect_wifi() // Connect to wifi network
{
...
void check_eeprom()
{
  ...
  if(vc_settings.eeprom_saved != 1) // Check if eeprom is fresh
  {
...
void check_wifi() // Check the wifi connection and retry if needed
{
  if (!WiFi.isConnected())
  {
...
void process_form(String &data, WiFiClient &client) // Process submitted form
{
  int start = 0;
  while (start < data.length()) {
    ...
    if      (key == "mode")        vc_settings.mode = value.toInt();
    else if (key == "recirc")      vc_settings.fan_recirc_enable = value.toInt();
    else if (key == "recirc_pwm")  vc_settings.fan_recirc_pwm = value.toInt();
    else if (key == "exhaust")     vc_settings.fan_exhaust_enable = value.toInt();
    else if (key == "exhaust_pwm") vc_settings.fan_exhaust_pwm = value.toInt();
    else if (key == "save")        save_to_eeprom = value.toInt();
...
void send_webpage(WiFiClient &client) // Send webpage to client
{
  if(client)
  {
